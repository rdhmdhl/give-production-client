    ...
    try {
      ...
      if (addFundsClicked) {
        if ((value > 0) & (value < 1000000)) {
          ...
          const response = await axios.post(
            `${config.apiUrl}/api/billing-settings-deposit`,
            updateBalance,
            {
              headers: {
                "x-auth-token": token,
          ...
          dispatch({
            type: "UPDATE_BALANCE",
            payload: response.data.new_balance,
          }); // Replace 'new_balance' with the actual property name in the response
      ...
      if (withdrawalClicked) {
        if ((value > 0) & (value < 1000000)) {
          ...
          const token = localStorage.getItem("token");
          const response = await axios.post(
            `${config.apiUrl}/api/billing-settings-withdrawal`,
            updateBalance,
            {
              headers: {
                "x-auth-token": token,
      ...
      await popupStatus(
        "You cannot withdraw more than what is in your account."
      );
    }
  };
    ...
    <div className={`modal ${isModalOpen ? "open" : ""}`} onClick={closeModal}>
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="balance-buttons">
          ...
          <button
            type="button"
            className="withdrawal-funds-btn"
            onClick={() => {
        ...
        {addFundsClicked || withdrawalClicked ? (
          <div className="input-container">
            ...
            <input
              ...
              // placeholder="value"
            />
            <button type="button" className="add-button" onClick={editBalance}>
              {addFundsClicked === true ? "Deposit" : "Withdrawal"}
            </button>
          </div>
        ) : null}
      </div>
