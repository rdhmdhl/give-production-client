...
export default function AcceptLink({ socket }) {
    ...
    const acceptorgive = async () => {
        try {
            ...
            let res = await axios.put(`${config.apiUrl}/api/accept-link/${linkId}`, {
                ...
                // update context so that the ui matches the server data
                const newBalance = balance - link.details.amount;
    ...
    return (
        ...
        {hasUserResponded
            ...
            <img className="profile-pic" src={currentUser.profilePicture ? currentUser.profilePicture : "/assets/no-profile-pic.webp"}/>
            <h3 className='own-link-message'>... you can`t use this link more than once</h3>
            </div> :
                isUserLink
                        ? <div className='own-link-container'>
                                <img className="profile-pic" src={linkUserData.profilePicture ? linkUserData.profilePicture : "/assets//no-profile-pic.webp"}/>
